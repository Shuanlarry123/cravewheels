...
/**
 * Grid of an author's (restaurant/creator) video posts, shown on their profile
 * or dashboard. Tapping a post opens its detail page (with comments).
 */
export default function PostFeed({ authorType, authorId, title = "Feed" }) {
  ...
  const load = async () => {
    try {
      const p = await base44.entities.Post.filter(
        ...
        <div className="grid grid-cols-2 gap-2">
          {posts.map((p) => (
            ...
            >
              ...
              <span className="absolute top-1.5 right-1.5 flex items-center gap-0.5 bg-black/50 px-1.5 py-0.5 rounded-full text-[10px] text-white">
